# Project 7: Azure Policy Implementation for Landing Zone Governance
...
Without governance controls, resource tagging was inconsistent, diagnostic logging coverage was patchy, and VM SKUs differed widely, creating operational and cost-management risks, including avoidable security exposure and financial uncertainty.

This project implemented Azure Policy as the initial governance layer to standardise configurations, enforce security baselines, and provide compliance visibility across the landing zone without disrupting existing workloads or the existing Infrastructure as Code (IaC) model.
...
- Deploy policies via Infrastructure as Code with pipeline automation for repeatability and auditability and peer-reviewed governance changes.
...
- Establish a dedicated `myorg-azuregovernance` repository for policies, RBAC, management group structure, budgets, and Defender configuration, separate from landing zone (`myorg-landing-zone`) and shared-services (`myorg-shared`) IaC.

## Architecture

- **Management Groups:** Root → Common → Platform/Landing Zone, providing policy inheritance to hub and spoke subscriptions and clearly separating platform and workload scopes.
...
- **Policy Assignments:** Applied at appropriate management group scopes per initiative theme, with exemptions managed via IaC so that all exceptions remain auditable in code.
- **Deployment:** Terraform modules defining policy definitions, initiatives, and assignments, orchestrated via Azure Pipelines from the central `myorg-azuregovernance` repo and using native `azurerm_policy_*` resources (no Azure Verified Modules exist for policy)
- **Reporting:** Azure Policy compliance dashboard integrated with Azure Monitor for remediation tracking.
- **Validation:** A dedicated test management group used to validate policy object deployment, scoping, inheritance, and Terraform–pipeline interaction, with no subscriptions required.
...
**Decision note (history):** Initial implementation ran alongside landing zone modules; governance was later refactored into a dedicated `myorg-azuregovernance` project to centralise policies, RBAC, budgets, and Defender settings and keep governance concerns separate from `myorg-shared` platform services.

## Operations

- Deployed policy definitions and initiatives to test management group via Azure Pipelines with PR approval gates from the `myorg-azuregovernance` repo.
...
- Configured policy exemption requests as pull requests against the Terraform `myorg-azuregovernance` repository, maintaining governance audit trail.
...
- Expand policy coverage to include Azure Defender for Cloud plans and regulatory frameworks (e.g. ISO 27001).
- Implement custom policy definitions for workload-specific naming conventions and resource lock requirements and to support subscription-level RBAC patterns aligned with landing zone design.
...
- **Deploy Azure Blueprints** for self-service subscription provisioning: Embeds `[MyOrg]-Common-Baseline` initiative assignment in new landing zone subscriptions, ensuring Day 1 governance compliance alongside standard RBAC, budgets, and core Defender plans.
- **Add budget alerts and cost management policies** linked to mandatory tagging: Configures £5k monthly warnings at landing zone scope, £50k at platform scope, driving financial accountability through existing tag data and keeping all budget artefacts in `myorg-azuregovernance`.

## Code References

Note: These references are placeholders and will be updated when the `/code` directory structure is finalised.

- `/code/myorg-azuregovernance/environments/` — Environment-scoped Terraform for management group targets (test, non-prod, prod).
- `/code/myorg-azuregovernance/modules/policies/` — Reusable Terraform modules for policy definitions, initiatives, assignments, and exemptions using azurerm_policy_* resources.
- `/code/myorg-azuregovernance/pipelines/` — Azure Pipelines YAML for governance deployment and validation.[END ADD]
...
Policy-as-code is implemented with native `azurerm_policy_definition`, `azurerm_policy_set_definition`, and `azurerm_policy_assignment` resources rather than Azure Verified Modules, as there are currently no verified modules dedicated to Azure Policy.
